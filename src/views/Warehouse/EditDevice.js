...
export default function EditDevice(props) {
    const { open, setOpen, officeStorageDevices, setOfficeStorageDevices, homeStorageDevices, setHomeStorageDevices, handleOpenAlert,
            userEmail, userFullName, existingDevice } = props;

    
    // Defining some variables to hold the user's input:
    const [name, setName] = useState(existingDevice.deviceName); // Text Input
    const [category, setCategory] = useState(existingDevice.category); // Text Input
    const [serial, setSerial] = useState(existingDevice.serial); // Text Input
    const [price, setPrice] = useState(existingDevice.price); // Number Input
    const [supplier, setSupplier] = useState(existingDevice.supplier); // Select
    const [storageType, setStorageType] = useState(existingDevice.storageType); // Select
    const [certificate, setCertificate] = useState(existingDevice.certificate); // Text Input
    const [certificateImage, setCertificateImage] = useState(existingDevice.certificateImage); // Image Input
    const [warrantyStartDate, setWarrantyStartDate] = useState(existingDevice.warrantyStart); // Date Picker
    const [warrantyPeriod, setWarrantyPeriod] = useState(existingDevice.warrantyPeriod); // Number Input
    const [notes, setNotes] = useState(existingDevice.labHistory); // Text Input { Expandable }
